...
A basic breakdown of the CAPP and KAPP manuscript is included in Appendix [INSERT REFERENCE]. 
...
\emph{Medical Plan} gives parents the option to set up reminders at particular times. It is divided according to the Traffic-Light system (See Appendix [INSERT REFERENCE]). A child has three different plans, such that an alarm that is set on the \emph{Healthy}-plan is not automatically set on the \emph{Sick}-plan.   
...
\paragraph{Information about the medicines}
\emph{Information about the medicines} gives general information about different medicines, what they do, and what they are used for. The three medicines that are currently in the system is Flutide, Seretide and Ventoline. Figures \ref{fig:information-1} and \ref{fig:information-2} shows screenshots from this functionality.
